        ...
        <div class="row">
            <div class="col">
                <div class="input-group d-inline-block w-auto">
                    ...
                    <input type="text" class="form-control d-inline-block w-auto" :value="todo.title" @input="todo.title = $event.target.value; updateTodo(todo)">
                </div>
                <select class="form-select w-auto d-inline-block" v-model="selected" @change="statusChanged">
                    <option v-for="item in taskStatuses" :key="item.id" :value='item.value'>{{ item.value }}</option>
                </select>
                <button class="btn btn-sm btn-danger ms-2" @click="deleteTodo">Delete</button>
                <button
                    type="button"
                    class="ms-2"
